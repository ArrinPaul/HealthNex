...
sequenceDiagram
    ...
    participant Backend
    participant Admin
    User->>App: Submits Health/Water Report
    App->>Backend: Encrypted Sync (Convex)
    Backend->>Admin: Alert Notified
    Admin->>Backend: Verify/Resolve Report
    Backend->>User: Status Update (Resolved/Investigating)
```
...
```mermaid
graph TD
    A[Voice/Text Input] --> B[Gemini AI v1.5]
    B --> C{NLP Processing}
...
1.  **Identity Initiation**: Personal and professional details submission.
2.  **Credential Upload**: Institutional ID or Medical License capture.
3.  **Admin Verification**: Manual review and promotion by Super Admin.
